...
(() => {
  ...
  const SFX = {
    ...
    power() { tone(300, 0.16, "sawtooth", 0.2, 950); },
    boost() { tone(170, 0.16, "sawtooth", 0.16, 440); },
    death() { tone(420, 0.5, "sawtooth", 0.28, 70); noise(0.4, 0.22, 320); },
  ...
  // موسيقى خلفية: أربيجيو نيون بسيط متكرّر + باص خفيف
  const NOTES = [220, 262, 330, 440, 330, 262, 196, 262];
  function musicStep() {
    if (!musicOn || muted || !ensure()) return;
    const t0 = ctx.currentTime, f = NOTES[mi % NOTES.length]; mi++;
    const o = ctx.createOscillator(), g = ctx.createGain();
    o.type = "triangle"; o.frequency.value = f;
    g.gain.setValueAtTime(0.0001, t0); g.gain.linearRampToValueAtTime(0.5, t0 + 0.04);
    g.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.42);
    o.connect(g); g.connect(musicGain); o.start(t0); o.stop(t0 + 0.45);
    if (mi % 4 === 1) { // باص
  ...
  window.Sound = {
    ...
    startMusic() { ensure(); resume(); if (musicOn) return; musicOn = true; musicTimer = setInterval(musicStep, 250); },
